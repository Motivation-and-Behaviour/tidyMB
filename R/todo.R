...
extract_comments <- function(path) {
  if (!grepl("md$", basename(path), ignore.case = TRUE)) {
    stop("can only be used on rmd, or md files")
  }
...
#' @param path output path. If null (default) the todo list is printed to
#' console
#' @details Reads all html comments from a rmarkdown file.
#' Those with |TODO| will be extracted and sorted by assignment.
#' Use @{name} e.g. '@james' to assign a task to someone.
#' @examples
#' library(tidyMB)
...
create_todo <- function(rmd = NULL, path = NULL) {
  ...
  todos <- unlist(comments[grepl("\\|TODO\\|",
    unlist(comments),
    ignore.case = TRUE
  )])
  # extract completions
